...
from encoder.blackbox_encoder import BlackboxEncoder, BlackboxEncoder2
from encoder.blackbox_encoder import WhiteboxEncoder, WhiteboxEncoder2
...
def generate_random_init_samples(args, num_samples=8000):
...
def main(args):
    ...
    km = KMeans(n_clusters=1024,random_state=0,n_init="auto")
    ...
    for name in args.encoder_list:
        ...
        if name == args.enc_tgt and not args.blackbox:
            if name != 'AdaFace':
                ...
                enc = WhiteboxEncoder2(enc, img_size=enc.img_size).to(args.device)
            # enc = WhiteboxEncoder(enc, img_size=enc.img_size).to(args.device)
        else:
            if name != 'AdaFace':
                enc = BlackboxEncoder(enc, img_size=enc.img_size).to(args.device)
            else:
                enc = BlackboxEncoder2(enc, img_size=enc.img_size).to(args.device)
    ...
    project_dir = os.path.abspath(os.path.dirname(__file__))
    init_samples = torch.load(f'{project_dir}/random_init_samples_8000.pth', map_location='cpu')
    ...
    init_latents2 = np.array(init_latents.clone().cpu().numpy())

    init_latents2 = km.fit(init_latents2)


    cluster_centers = torch.tensor(init_latents2.cluster_centers_, dtype=torch.float32).to(args.device)
    labels = init_latents2.labels_
    ...
    for cnt in range(num_targets):
        init_feats4 = torch.FloatTensor().to(args.device)
        init_feats5 = torch.FloatTensor().to(args.device)

        for beg in tqdm(range(0, cluster_centers.shape[0], args.batch_size)):
            end = min(args.num_inits, beg + args.batch_size)
            latents = cluster_centers[beg:end]
            with torch.no_grad():
                init_image = stgan(latents)
                feat = enc_dict[args.enc_tgt]['enc'](init_image, flip=True)
                init_feats4 = torch.cat((init_feats4, feat), dim=0)
        # for i in range(cluster_centers.shape[0]):
        #     with torch.no_grad():
        #         init_image = stgan(cluster_centers[i].unsqueeze(0))
        #         feat = enc_dict[args.enc_tgt]['enc'](init_image, flip=True)
        #         init_feats4 = torch.cat((init_feats4, feat), dim=0)
        ...
        init_latents3 = torch.FloatTensor().to(args.device)
        init_feats2 = torch.FloatTensor().to(args.device)
        ...
        with torch.no_grad():
            ...
            if args.fixed_init:
                ...
                cosine = cosine_similarity(enc_dict[args.enc_tgt]['target'], init_feats4).squeeze()

                _, idx = cosine.topk(args.topk1)

                for i in range(args.num_inits):
                    for j in range(len(idx.cpu().numpy())):
                        if idx.cpu().numpy()[j] == labels[i]:
                            init_latents3 = torch.cat((init_latents3, init_latents[i].unsqueeze(0)))
                            init_feats2 = torch.cat((init_feats2, init_feats[i].unsqueeze(0)))
                # init_image = stgan(init_latents3)
                # init_feats2 = enc_dict[args.enc_tgt]['enc'](init_image, flip=True)

                print(init_feats2.shape)
                ...
                for beg in tqdm(range(0, init_latents3.shape[0], args.batch_size)):
                    end = min(args.num_inits, beg + args.batch_size)
                    latents = init_latents3[beg:end]
                    init_image = stgan(latents)
                    feat = enc_dict[args.enc_tgt]['enc'](init_image, flip=True)
                    init_feats5 = torch.cat((init_feats5, feat), dim=0)

                cosine = cosine_similarity(enc_dict[args.enc_tgt]['target'], init_feats5).squeeze()
                _, idx = cosine.topk(args.topk)
                args.init_latent = init_latents3[idx].mean(dim=0, keepdim=True)  # latent ensembling
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_iters', default=100, type=int, help='number of iteration per reconstruction')
    parser.add_argument('--ndirs', default=9, type=int, help='number of perturbation directions for ZOGE')
    ...
    parser.add_argument('--num_inits', default=8000, type=int, help='number of init latents')
    parser.add_argument('--topk', default=5, type=int, help='topk for latent ensembling')
    parser.add_argument('--topk1', default=80, type=int, help='topk for latent ensembling')
    ...
    parser.add_argument('--encoder_list', default=['FaceNet', 'VGGNet19', 'SwinTransformer', 'AdaFace', 'MagFace', 'ArcFace'], type=list,
    ...
    args.save_dir = f'results/{args.dataset}/{args.enc_tgt}_best_8000_1024'
